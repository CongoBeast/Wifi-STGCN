...
class spatio_conv_layer(nn.Module):
    def __init__(self, ks, c, Lk):
        ...
        self.attention_weights = nn.Parameter(torch.FloatTensor(c, 1))
    ...
    def reset_parameters(self):
        ...
        init.uniform_(self.attention_weights, -bound, bound)
    ...
    def forward(self, x):
        ...
        attention_scores = torch.matmul(x_c, self.attention_weights)
        attention_weights = torch.softmax(attention_scores, dim=-1)
        x_c_attended = torch.einsum('bitkn,bitk->bitn', x_c, attention_weights)
        
        # Graph convolution
        x_gc = torch.einsum("iok,bitn->botn", self.theta, x_c_attended) + self.b
...
class fully_conv_layer(nn.Module):
    def __init__(self, c):
        ...
        self.conv = nn.Conv2d(c, 1, 1 , bias=True)
