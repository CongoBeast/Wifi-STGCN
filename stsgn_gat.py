...
class temporal_conv_layer_s(nn.Module):
    def __init__(self, kt, c_in, c_out, act="relu"):
        super(temporal_conv_layer_s, self).__init__()
        self.kt = kt
        self.act = act
        self.c_out = c_out
        self.align = align(c_in, c_out)
    ...
    def forward(self, x):
        ...
        if self.act == "GLU":
            x_conv = self.conv(x)
            return (x_conv[:, :self.c_out, :, :] + x_in) * x_conv[:, self.c_out:, :, :]
        if self.act == "sigmoid":
            return self.conv(x) + x_in
        return self.conv(x) + x_in
...
class st_conv_block(nn.Module):
    def __init__(self, ks, kt, n, c, p, Lk):
        ...
        self.tconv1 = temporal_conv_layer_s(kt, c[0], c[1], "GLU")
